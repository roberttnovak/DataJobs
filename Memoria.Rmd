...
## Tratamiento de las distintas variables del dataset 
...
En el análisis ue hemos realizado, hemos observado que  la variable `Salary Estimate Med` tiene una distribución de valores bastante consistente, lo cual podemos comprobar dado el pequeño tamaño de su rango intercuartílico (caja). No obstante, vemos como existen puntos fuera de este rango y de los *whiskers*, los conocidos como *outliers*. En concreto, son dos puntos: `r salary_outliers[1]` y `r salary_outliers[2]`. 
...
## Análisis entre las variables categóricas y la variable cuantitativa `Salary Estimate Med`

A continuación, vamos a comprobar las diferencias entre los distintos grupos existentes en el dataset. 

La cuestión principal es ¿Existen diferencias salariales significativas entre los distintos trabajos que hemos definido? Primero, representemos esto gráficamente 
...
Se puede apreciar que los salarios se distribuyen de forma bastante uniforme. Mediante el test de kruskal, un test no paramétrico cuya hipótesis inicial es que todas las muestras provienen de la misma distribución hemos comprobado formalmente que esto efectivamente es así, pues el p valor es `r kruskal_job`. Es decir, un p valor demasiado alto como para poder rechazar la hipótesis nula. Dicho de otro modo, no podemos descartar que los salarios dados por los distintos puestos provengan de una misma distribución y , por tanto, haya diferencias significativa entre ellas. 

Ocurre lo mismo para las otras dos variables ordinales que hacen referencia al tamaño de la empresa y al revenue. Pues, en ambos casos al replicar el test los p valores son `r kruskal_size` y `r kruskal_revenue` respectivamente, Así como de la variable categórica que representa la industria con un p-valor de `r kruskal_industry`

A continuación se puede ver una tabla resumen con los resultados obtenidos
...
```{r echo=F}
corr <- cor(clean_data$Rating, clean_data$`Salary Estimate Med`, use="complete.obs")
```
...
La correlación que nos ha salido entre estas dos variables es de `r corr`. Es decir, no parece que estén correlacionadas. 
