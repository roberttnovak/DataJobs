...
La cuestión principal es **¿Existen diferencias salariales significativas entre los distintos trabajos que hemos definido?** Si representamos esto gráficamente, se puede apreciar que los salarios se distribuyen de forma bastante uniforme:
...
Mediante el test de kruskal, un test no paramétrico cuya hipótesis inicial es que todas las muestras provienen de la misma distribución hemos comprobado formalmente que esto efectivamente es así, pues el p valor es `r kruskal_job`. Es decir, un p valor demasiado alto como para poder rechazar la hipótesis nula. Dicho de otro modo, no podemos descartar que los salarios dados por los distintos puestos provengan de una misma distribución y , por tanto, haya diferencias significativa entre ellas. 
